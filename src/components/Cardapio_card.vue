    ...
    methods: {
        ...
        async saveNewMenu() {
            if (this.dia == null) {
                ...
                for (let i in this.guarnições_form) { array_form_foods.push(this.guarnições_form[i].food.toLowerCase()) }

                //Filtrando campo food vazios
                const filtered_foods = array_form_foods.filter(food => food.length >= 1)
